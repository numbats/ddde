...
starts <- bikes$trips |>
  count(Start.station) |>
  rename(station = Start.station,
         nstart = n)
ends <- bikes$trips |>
  count(End.station) |>
  rename(station = End.station,
         nend = n)
s_e <- full_join(starts, ends)
# Not the same so need to fill
miss1 <- s_e |>
  filter(is.na(nstart)) |>
  pull(station)
miss2 <- s_e |>
  ...
  pull(station)
b_fill1 <- bikes$trips |>
    filter(Start.station %in% miss2) |>
    rename(Start.station = End.station,
         End.station = Start.station) |>
  mutate(n = 0, minlength = 0) |>
  select(Start.station, End.station, n, minlength)
b_fill2 <- bikes$trips |>
    filter(End.station %in% miss1) |>
    rename(Start.station = End.station,
         End.station = Start.station) |>
  mutate(n = 0, minlength = 0) |>
  select(Start.station, End.station, n, minlength)

bikes$trips <- bind_rows(bikes$trips, b_fill1, b_fill2)
bikes_trips_full <- bikes$trips |>
  select(Start.station, End.station, n) |>
  mutate(Start.station = as.character(Start.station),
         End.station = as.character(End.station)) |>
  complete(Start.station, End.station, fill = list(n = 0))

bike_order <- bikes_trips_full |>
  group_by(Start.station) |>
  summarise(n = sum(n)) |>
  arrange(desc(n))
  
p <- bikes_trips_full |>
  mutate(Start.station = factor(Start.station,
              levels=bike_order$Start.station),
         End.station = factor(End.station,
              levels=bike_order$Start.station)) |>
  ggplot( 
       aes(x=Start.station,
...
bikes_connect <- bikes_trips_full |> 
  filter(n > 0) |>
  mutate(Start.station = factor(Start.station,
              levels=bike_order$Start.station),
         End.station = factor(End.station,
              levels=bike_order$Start.station)) |>
  rename(from = Start.station, to = End.station) 
bikes_graph <- bikes_connect |> as_tbl_graph() 

bikes_g_plt <- ggraph(bikes_graph, layout = 'tree', circular=TRUE) + 
  geom_edge_link() +
  geom_node_point() 

bikes_nodes <- tibble(
  x = bikes_g_plt$data$x,
  y = bikes_g_plt$data$y,
  name = levels(bikes_connect$from))
bikes_connect <- bikes_connect |>
  mutate(from_num = as.numeric(from),
         to_num = as.numeric(to))
bikes_edges <- tibble(
  x = bikes_g_plt$data$x[bikes_connect$from_num],
  y = bikes_g_plt$data$y[bikes_connect$from_num],
  xend = bikes_g_plt$data$x[bikes_connect$to_num],
  yend = bikes_g_plt$data$y[bikes_connect$to_num],
  n = bikes_connect$n
)
